...
const Chart = ({app,
    ...
    const CustomTick = tick => {
        ...
        if ((tickRotation > 0) && (tickRotation < 180)) {
            ...
            return (<g transform={`translate(${tick.x},${tick.y + 30})`}>
                {showTickLine &&
                    <line stroke={overrideTickColor ? tickColor: DEFAULT_TICK_BG_COLOR} strokeWidth={1.5} y1={-32} y2={-12}/>
                }
    ...
    const AreaLayer = ({series, xScale, yScale, innerHeight}) => {
       ...
       const sortedData = measureMinValues.sort((a, b) => {
         ...
         let lower = areaShadingCriteria == 'CUSTOM_BETWEEN_TWO_LINES' && areaLowerBound ? areaLowerBound : sortedData[0].measure
         let upper = areaShadingCriteria == 'CUSTOM_BETWEEN_TWO_LINES' && areaUpperBound ? areaUpperBound : sortedData[sortedData.length - 1].measure
       ...
       return (<>{showLegends && chartLegends.map(legend => {
            return (
                <div className={"legend item"} onClick={() => toggle(legend.id)}>
                    ...
                    <span className={legendCheckBack == true ? 'checkmark-with-bg' : 'checkmark'}
                          style={{backgroundColor: (legendCheckBack === true) ? legend.color : "transparent"}}></span>
    ...
    const chartLegends = options.data.map(d => ({id: d.id, label: d.id, color: colorGenerator.getColor(d.id, d)}))
