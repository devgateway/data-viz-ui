...
const toOptions = (items, locale) =>
    items
        ? items
            ...
            .map((i) => {
                const text =
                    locale && i.labels && i.labels[locale.toUpperCase()]
                        ? i.labels[locale.toUpperCase()]
                        : i.value;
                return {
                    key: i.id,
                    value: i.id,
...
const FilterSelectorBox = connect(
    ...
    const getSelected = () => {
        if (filterType == FILTER_TYPE_SINGLE_SELECT) {
            const selectedItem =
                current && current[0]
                    ? options.filter((v) => v.value == current[0])[0]
                    : null;
    ...
    ({
        ...
        useEffect(() => {
            const current = options
                .filter(
                    (v) =>
                        (v.position > start || v.position === start) &&
