...
class Map extends React.Component {
        ...
        if (zoomOnFilterField) {
            ...
            if (prevAppliedFilters) {
                Object.keys(prevAppliedFilters).forEach(k => {
                    prevAppliedItems.push(...prevAppliedFilters[k].filter(v => v != Number.MIN_SAFE_INTEGER))
                });
            }
            if (appliedFilters) {
                Object.keys(appliedFilters).forEach(k => {
                    appliedItems.push(...appliedFilters[k].filter(v => v != Number.MIN_SAFE_INTEGER))
                });
            }
            //filters reset
    ...
    resizeLabels() {
        ...
        labels.attr('x', (d) => {
            ...
            const width = this.getLabelBoxWidth(d)
            return d3.event.transform.k > 1 ? width / d3.event.transform.k : width
        })
            .attr("height", (d) => {
                const height = this.getLabelBoxHeight(d)
    ...
    resizePointLabels() {
        ...
        labels.attr('x', (d) => {
            ...
            /*.attr("height", (d) => {
                const height = this.getLabelBoxHeight(d) + 20
                return d3.event.transform.k > 1 ? height / d3.event.transform.k : height
            })*/
    ...
    drawPoints(sortedFeatures, filterUpdated) {
        ...
        if (showShadingLayerLabels == SHOW_ALL) {
            ...
            points = sortedFeatures.filter(p => p.geometry && p.geometry.type == 'Point' && p.properties.hasDataRow)
        }
                                    ...
                                    {app != 'csv' && showOverallValue &&
                                        <Grid.Column textAlign={"left"} width={4}>
                                            <div className="national-average-div">
                                                ...
                                                className="national-avg-value">{formatContent(valueFormat, {value: nationalAverage}, intl, noDataText)}</span>
                                            </div>
                                        </Grid.Column>
                                    }
